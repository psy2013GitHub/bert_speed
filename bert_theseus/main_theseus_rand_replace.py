...
def init_theseus(init_checkpoint):
    if init_checkpoint:
        tvars = tf.trainable_variables()
        assignment_map, suc_assignment_map, initialized_variable_names = theseus_modeling.get_assignment_map_from_checkpoint_for_theseus(tvars, init_checkpoint)
        tf.train.init_from_checkpoint(init_checkpoint, assignment_map)
        tf.train.init_from_checkpoint(init_checkpoint, suc_assignment_map)
...
def model_fun(features, labels, mode, params):
    ...
    # 从保存的模型里初始化
    init_theseus(params.get('bert_init_checkpoint', None))
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        train_op = create_optimizer(loss=loss, learning_rate=params['optimizer']['learning_rate'], trainable_regexps=['succ_',])
...
def main():
    ...
    params = {
        ...
        'RESULT_PATH': './results_theseus/',
        'DATADIR': '~/.datasets/CLUE/tnews_public/',
        'log_path': './results_theseus/main.log',
        'model_pb_dir': './results_theseus/theseus_pb/'
    }
    ...
    hook = tf.estimator.experimental.stop_if_no_decrease_hook(
        estimator, 'loss', 200, min_steps=0, run_every_steps=100, run_every_secs = None)
    ...
    sent_email('theseus_rand_replace done', 'theseus_rand_replace done')
